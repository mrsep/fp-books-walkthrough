  ...
  (loop [loc (circus-zip (construct-node root (program-weights root)))]
    (if (:sum (zip/node loc))
      (if (zip/end? loc)
        loc
        (recur (zip/up loc)))
      (let [p-childs (program-childs (:id (zip/node loc)))
            z-childs (zip/children loc)
            diff     (clojure.set/difference p-childs z-childs)]
        (cond
          (and (empty? diff)
               (not (empty? z-childs))) (zip/edit loc #(assoc (zip/node %1) :sum (reduce + (map :sum %2)))
                                                  (zip/children loc))
          (empty? diff) (recur (zip/up loc))
          :true (recur (zip/node (zip/append-child loc (construct-node (first diff)
